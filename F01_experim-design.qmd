...
Generally, we categorize variables as either independent, dependent, or controlled variables. The **dependent** or **response** variable is the variable you expect to be affected and are therefore measuring in your experiment[^f01_experim-design-6]. By contrast, the **independent** or **explanatory** variable is the variable or phenomena that you think will affect your dependent variable[^f01_experim-design-7]. Any remaining variables are what we would consider **constants** or **controlled variables**. These are factors that we will want to intentionally keep constant so they do not effect the outcome.

[^f01_experim-design-6]: It is not uncommon to measure more than one dependent variable in an experiment. Frequently you will measure the outcome in more than one way.

[^f01_experim-design-7]: Having only one independent variable makes the interpretation of the an experiment straightforward. However, including two or more independent variables allows us to test not only the effect of each explanatory variable but also how they interact, i.e. how they modify each other's effects.
...
Our experiment should include at least two groups or **treatments**[^f01_experim-design-8], the **control** and the **experimental** groups. The groups should be completely the same, with the exception that an **experimental group** receives a treatment while the **control** does not. Comparing these two groups allows us to determine if the treatment has had an effect. You should always include a control group to have a baseline of what the outcome should look like if there is no effect (**negative** control) or if there is an effect (**positive** control)[^f01_experim-design-9]. Depending on the complexity of your hypothesis and the number of dependent and independent variables you are testing, your design may comprise several experimental groups, each with a different treatment.

[^f01_experim-design-8]: We use the term **treatment** to describe a set of conditions.

[^f01_experim-design-9]: It's important to keep in mind that positive and negative have nothing to do with "good" or "bad" in this context but rather presence/absence of an effect.
...
The type of plant being grown and potentially snacked on by slugs is also a variable that we probably want to control. The most straightforward set up would be to identify a plant that we know slugs like to eat[^f01_experim-design-10] and then have each plot be set up in an identical way.

[^f01_experim-design-10]: This is important because otherwise we don't know whether there was no slug damage/presence because of that plant itself or the Allicin treatment.
